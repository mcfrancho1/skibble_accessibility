...
import '../../../src/constants/auth_strings.dart';
...
class FirebaseEmailLinkHandler {
  ...
  Future<void> _signInWithEmail(String link) async {
    try {
      ...
      final SkibbleUser? user = await auth.currentUser();
      if (user != null) {
        _errorController.add(EmailLinkError(
      ...
      if (auth.isSignInWithEmailLink(link)) {
        await auth.signInWithEmailAndLink(email, link);
      } else {
        _errorController.add(EmailLinkError(
