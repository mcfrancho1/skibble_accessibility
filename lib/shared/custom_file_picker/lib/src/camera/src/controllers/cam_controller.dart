...
import '../../../../../../camera_utils/video_view.dart';
...
class CamController extends ValueNotifier<CamValue> {
  ...
  ///
  /// Take picture
  Future<SkibbleEntity?> takePicture(BuildContext context) async {
    ...
    try {
      ...
      if (entity != null) {
        final skibbleEntity = entity.toSkibble.copy(
          pickedThumbData: bytes,
          pickedFile: file,
          isRecentlyCreated: true
        );
        if (navigator.mounted) {
          navigator.pop([skibbleEntity]);
        }
        return skibbleEntity;
      }
      else {
        final exception = CameraException(
          'takePicture',
      ...
      ///
      // var result = await CustomCropper.cropImage(
      //   file.path,
      //   onDone: (res) async {
      //     await CustomCropper.cropImage(
      //         file.path,
      //         onDone: (res) async {
      //           if(res != null) {
      //             processFile(navigator, path: res.path, file: file);
      //             final AssetEntity? imageEntityWithPath = await PhotoManager.editor.saveImageWithPath(
      //               res.path, // Use the absolute path of your source file, it's more like a copy method.
      ...
      //             }
      //
      //             if(imageEntityWithPath != null) {
      //
      //               navigator.pushAndRemoveUntil(MaterialPageRoute(
      //                   builder: (context) =>
      //                       EditPostView(skibEntities: [SkibEntity(assetEntity: imageEntityWithPath, isRecentlyCreated: true)])),
      //                   ModalRoute.withName('/'));
      //             }
      //
      //             else {
      ...
      //
      //           else {
      //             processFile(navigator, path: file.path, file: file);
      //           }
      //         }
      //     );
      //   }
      ...
      //   await controller.pausePreview();
      //   final route = SlideTransitionPageRoute<SkibbleEntity?>(
      ...
      //   }
        ...
        // if (entity != null) {
        //   final drishyaEntity = entity.toDrishya.copy(
        //     pickedThumbData: bytes,
        //     pickedFile: file,
        //   );
        //   if (navigator.mounted) {
  ...
  processFile(navigator, {String? path, File? file}) async {
    if(path != null) {
      // final AssetEntity? imageEntityWithPath = await PhotoManager.editor.saveImageWithPath(
      ...
      // if(imageEntityWithPath != null) {
        // File? file = await imageEntityWithPath.file;
        navigator.pushAndRemoveUntil(MaterialPageRoute(
            builder: (context) =>
                EditPostView(skibEntities: [SkibEntity(file: File(path), fileType: AssetType.image, isRecentlyCreated: true)])),
            ModalRoute.withName('/'));
      // }
  ...
  void switchCameraDirection(CameraLensDirection direction) {
    ...
    try {
      ...
      final exception = CameraException('switchingCameraLenses', e.toString());
      value = value.copyWith(error: exception);
      return;
    }
  }
  ...
  Future<SkibbleEntity?> stopVideoRecording(
            ...
            MaterialPageRoute(
                builder: (context) => VideoViewPage(
                  ...
                  onDone: () async{
                      ...
                      final File videoFile = File(xfile.path);
                      ...
                      //   title: 'skibble_vidskib_${DateTime.now().millisecondsSinceEpoch}.mp4',
                      // );
                      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => EditPostView(skibEntities: [SkibEntity(file: videoFile, fileType: AssetType.video, isRecentlyCreated: true)])), ModalRoute.withName('/'));

                      // Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => EditPostView(assetEntities: [videoEntity!])));
                  },
                )));
        ///Old Code
        // if (createEntity) {
